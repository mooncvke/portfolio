...
function getTracer() {
  return _api.trace.getTracer(name, version);
}
  ...
  return trace(
    async () => {
      if (!shouldUseDevelopmentCache)
        return null;
      latestRequest != null ? latestRequest : latestRequest = fetchWithCachedResponse(
      ...
      void latestRequest.then((resolved) => {
        embeddedEdgeConfigPromise = Promise.resolve(resolved);
      }).finally(() => {
        latestRequest = null;
      });
      embeddedEdgeConfigPromise != null ? embeddedEdgeConfigPromise : embeddedEdgeConfigPromise = latestRequest;
...
exports.clone = clone3; exports.createClient = createClient; exports.digest = digest; exports.get = get; exports.getAll = getAll; exports.has = has; exports.parseConnectionString = parseConnectionString;
